...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtTerrainFeature : MonoBehaviour
	{
		...
		{
			#if UNITY_EDITOR
				if (heightmap != null && heightmap.isReadable == false)
				{
			...
			if (heightmap != null && heightmap.isReadable == true)
			{
				...
				var scaleI = SgtHelper.Reciprocal(scale);
				var aspect = SgtHelper.Divide(heightmap.height, heightmap.width);
				...
				job.Points       = points;
				job.Heights      = heights;
				job.Rotation     = math.mul(float3x3.EulerXYZ(math.radians(rotation)), float3x3.Scale(scaleI * aspect, scaleI, 1.0f));
